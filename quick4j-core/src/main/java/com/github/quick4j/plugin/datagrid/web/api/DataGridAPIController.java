...
public class DataGridAPIController {
  ...
  @Resource
  private SimpleCrudService<BaseEntity> simpleCrudService;
...
//            method = RequestMethod.POST,
  ...
  public JsonMessage loadDataFor(@PathVariable("name") String name,
    ...
    if (StringUtils.isNotBlank(sortName) && !sortName.contains(",")) {
      pageRequest.setSort(new Sort(sortName, sortOrder.toUpperCase()));
    }

    DataPaging dataPaging = criteria.selectList(pageRequest);
  ...
  private Map<String, Object> wrapRequestMap(HttpServletRequest request) {
    ...
    for (Iterator<Map.Entry<String, String[]>>
             iterator =
             request.getParameterMap().entrySet().iterator(); iterator.hasNext(); ) {
