...
<script>
                ...
                $.ajax({
                           ...
                           success: function (data) {
                               if (data.status == 200) {
                                   ...
                                   $.messager.alert('错误', data.status + '<br>' + data.message,
                                                    'error');
                               }
                           },
                           error: function () {
    ...
    function doBuildButton(value, row, index) {
        return '<div class="icon-edit datagrid-operation-button" title="编辑" onclick="doEdit('
                       + index + ')"></div>' +
               '<div class="icon-remove datagrid-operation-button" title="删除" onclick="doDelete('
                       + index + ')"></div>';
    }
